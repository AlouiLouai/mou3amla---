...
| Identity verification | Didit | KYC launch/session creation and webhook syncing live under `src/app/api/didit/**`; the app also has a server-side status re-sync path for pending sessions under `src/features/onboarding/server/**`. White-label styling is primarily configured in Didit's dashboard, while the workflow id can be supplied via `DIDIT_WORKFLOW_ID` and is not treated as a secret. |
