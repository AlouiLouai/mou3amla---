...
| Live notifications | Supabase Realtime (Postgres Changes) | `useRealtimeNotifications` subscribes to INSERT events on `public.notifications`, enabled via `alter publication supabase_realtime add table public.notifications`. RLS-scoped automatically. This is the only Realtime-enabled table - other flows (nearby handoff) still poll deliberately, see [06-conventions.md](./06-conventions.md). |
