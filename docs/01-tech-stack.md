...
| Analytics / error tracking | PostHog (`posthog-js` client + OTel server logs), optional | Entirely opt-in on `NEXT_PUBLIC_POSTHOG_KEY` (both client and server). Client: `analytics-provider.tsx` - manual pageview capture, `capture_exceptions` autocapture, explicit `captureException(...)` in `error.tsx`/`global-error.tsx`, session recording masks all `<input>` elements. `next.config.ts`'s CSP only widens to allow `*.posthog.com` when the key is set. Server: `src/instrumentation.ts` registers an OTel logger (`src/lib/posthog-otel-logger.ts`) that every `logger.*` call in `src/lib/logger.ts` also ships to PostHog Logs, through the same key-based redaction - no separate/raw logging path. |
